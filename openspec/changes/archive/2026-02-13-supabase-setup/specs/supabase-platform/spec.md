...
The system SHALL create and manage the Supabase project using the official `supabase/supabase` Terraform provider.
...
The system SHALL store the Supabase admin connection URL in GCP Secret Manager for use by the app-database module.
...
The supabase directory SHALL have Terramate configuration for GCS backend, SOPS, and Supabase providers.

#### Scenario: Stack generates correct providers
- **WHEN** terramate generate is run
- **THEN** the stack generates _backend.tf with GCS backend prefix "tfstate/supabase/dmikalova"
- **AND** the stack generates _providers.tf with google, sops, and supabase providers
