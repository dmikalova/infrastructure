...
The `gcp/` directory SHALL contain a `terramate.tm.hcl` file that configures
Terramate for the GCP stacks.
...
Terramate SHALL discover stacks under `gcp/infra/` and `gcp/project/`
...
- **WHEN** a new project directory is created (e.g.,
  `gcp/project/email-unsubscribe/`)
- **THEN** it appears in the stack list after adding stack configuration

## Requirement: Global variables

Terramate SHALL define global variables for shared configuration (project ID,
region, Fabric module version).
...
- **THEN** each stack receives a `_backend.tf` file with GCS backend
  configuration
...
The `gcp/` directory SHALL separate infrastructure foundation from deployed
workloads.
